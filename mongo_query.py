...
def build_query(settings, request):
    ...
    for key,value in request.arguments.items():
        ...
        key, value = operator_value(key, value)  # 'field=$foo:bar' ---> 'field', {'$foo': ['bar']}
        arguments[key] = value
    ...
    for key in set(list(default_query_filter.keys()) + list(arguments.keys())):
        ...
        logging.debug(f"{prefix} - key: {key!r} is: {arguments.get(key)!r} in arguments")

        # Key does exist in arguments and default value is None
        if value is None and arguments.get(key) is not None:
            logging.debug(f"{prefix} - setting: {key!r} to: {arguments.get(key)!r}, None in query_filter")
        ...
        # Keys in the default value are preferred when keys overlap
            ...
            logging.debug(f"{prefix} - merged: {key!r} as: {merged_value!r} in query_filter")
            query_filter[key] = merged_value

        # Key does exist in arguments
        # Overwrite with the default value 
        else:
            logging.debug(f"{prefix} - overwriting: {key!r} to: {value!r} in query_filter")
            query_filter[key] = value
...
def set_option_projection(request, query) -> dict:
    ...
    if query.get('projection') is None and request.arguments.get('projection') is not None:
